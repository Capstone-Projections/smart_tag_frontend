...
export const AppEntry = styled.TextInput<AppEntryProps>`
    fontFamily: Poppins;
    font-size:  ${props => props.type === 'popup'?'8.05px': props.type ==='otp'? '25px':'18px'};
    color: #000;
    ...
    padding:  ${props => props.type === 'popup'?'4px': props.type ==='otp'? '10px':'10px'};
    ...
    flex: none;
    flex-grow: 0;
    marginBottom: ${props => props.type === 'popup'?'8.75px': props.type ==='otp'? '53px':'24px'};
    marginTop: ${props => props.type === 'popup'?'1.65px': props.type ==='otp'? '53px':'12px'};
    align-items: center;
`;
...
export const Entry = ({type,text,textWeight,textType,placeholder}:AppEntryProps) => (
    <View >
